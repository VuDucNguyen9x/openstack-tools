...
function nova_create_info () {
	...
	openstack endpoint create --region RegionOne compute public http://$CTL__IP:8774/v2.1
	openstack endpoint create --region RegionOne compute internal http://$CTL__IP:8774/v2.1
	openstack endpoint create --region RegionOne compute admin http://$CTL__IP:8774/v2.1
	...
	openstack endpoint create --region RegionOne placement public http://$CTL__IP:8778
	openstack endpoint create --region RegionOne placement internal http://$CTL__IP:8778
	openstack endpoint create --region RegionOne placement admin http://$CTL__IP:8778
}
...
nova_config () {
	...
	ops_add $novafile api_database \
		connection mysql+pymysql://nova:$NOVA_API_DBPASS@$CTL__IP/nova_api

	ops_add $novafile database \
		connection mysql+pymysql://nova:$NOVA_DBPASS@$CTL__IP/nova

	ops_add $novafile DEFAULT \
		transport_url rabbit://openstack:$RABBIT_PASS@$CTL__IP

	ops_add $novafile api auth_strategy keystone

	ops_add $novafile keystone_authtoken auth_uri http://$CTL__IP:5000
	ops_add $novafile keystone_authtoken auth_url http://$CTL__IP:5000
	ops_add $novafile keystone_authtoken memcached_servers $CTL__IP:11211
	...
	ops_add $novafile DEFAULT my_ip $CTL__IP
	...
	ops_add $novafile glance api_servers http://$CTL__IP:9292
	...
	ops_add $novafile placement auth_url http://$CTL__IP:5000/v3
