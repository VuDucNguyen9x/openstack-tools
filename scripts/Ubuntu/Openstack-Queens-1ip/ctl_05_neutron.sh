...
function neutron_create_info () {
	...
	openstack endpoint create --region RegionOne network public http://$CTL__IP:9696
	openstack endpoint create --region RegionOne network internal http://$CTL__IP:9696
	openstack endpoint create --region RegionOne network admin http://$CTL__IP:9696
}
...
function neutron_config_server_component () {
	...
	ops_add $neutronfile database connection mysql+pymysql://neutron:$NEUTRON_DBPASS@$CTL__IP/neutron
	...
	ops_add $neutronfile DEFAULT transport_url rabbit://openstack:$RABBIT_PASS@$CTL__IP
	...
	ops_add $neutronfile keystone_authtoken auth_uri http://$CTL__IP:5000
	ops_add $neutronfile keystone_authtoken auth_url http://$CTL__IP:5000
	ops_add $neutronfile keystone_authtoken memcached_servers $CTL__IP:11211
	...
	ops_add $neutronfile nova auth_url http://$CTL__IP:5000
...
function neutron_config_linuxbridge () {
	...
	ops_add $linuxbridgefile vxlan local_ip $CTL_OVERLAY_INTERFACE_IP
...
function neutron_config_metadata () {
	...
	ops_add $metadatafile DEFAULT nova_metadata_host $CTL__IP
...
function neutron_config_compute_use_network () {
	...
	ops_add $novafile neutron url http://$CTL__IP:9696
	ops_add $novafile neutron auth_url http://$CTL__IP:5000
