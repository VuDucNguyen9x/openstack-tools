...
function keystone_config () {
	...
	connection mysql+pymysql://keystone:$KEYSTONE_DBPASS@$CTL_IP/keystone
...
function keystone_bootstrap () {
	keystone-manage bootstrap --bootstrap-password $ADMIN_PASS \
	  --bootstrap-admin-url http://$CTL_IP:5000/v3/ \
	  --bootstrap-internal-url http://$CTL_IP:5000/v3/ \
	  --bootstrap-public-url http://$CTL_IP:5000/v3/ \
...
function keystone_create_domain_project_user_role () {
	...
	export OS_AUTH_URL=http://$CTL_IP:5000/v3
