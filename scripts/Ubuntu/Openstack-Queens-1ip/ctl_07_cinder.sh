...
function cinder_user_endpoint() {
	...
	openstack endpoint create --region RegionOne volumev2 public http://$CTL_IP:8776/v2/%\(tenant_id\)s
	openstack endpoint create --region RegionOne volumev2 internal http://$CTL_IP:8776/v2/%\(tenant_id\)s
	openstack endpoint create --region RegionOne volumev2 admin http://$CTL_IP:8776/v2/%\(tenant_id\)s

	openstack endpoint create --region RegionOne volumev3 public http://$CTL_IP:8776/v3/%\(tenant_id\)s
	openstack endpoint create --region RegionOne volumev3 internal http://$CTL_IP:8776/v3/%\(tenant_id\)s
	openstack endpoint create --region RegionOne volumev3 admin http://$CTL_IP:8776/v3/%\(tenant_id\)s
...
function cinder_install_config() {
	...
	ops_add $ctl_cinder_conf DEFAULT transport_url rabbit://openstack:$RABBIT_PASS@$CTL_IP
	ops_add $ctl_cinder_conf DEFAULT auth_strategy keystone
	ops_add $ctl_cinder_conf DEFAULT my_ip $CTL_IP

	ops_add $ctl_cinder_conf database connection  mysql+pymysql://cinder:$CINDER_DBPASS@$CTL_IP/cinder

	ops_add $ctl_cinder_conf keystone_authtoken auth_uri http://$CTL_IP:5000
	ops_add $ctl_cinder_conf keystone_authtoken auth_url http://$CTL_IP:5000
	ops_add $ctl_cinder_conf keystone_authtoken memcached_servers $CTL_IP:11211
