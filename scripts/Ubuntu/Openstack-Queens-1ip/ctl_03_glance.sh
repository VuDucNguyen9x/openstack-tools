...
glance_create_service () {
	...
	openstack endpoint create --region RegionOne \
	  image public http://$CTL_IP:9292
	openstack endpoint create --region RegionOne \
	  image internal http://$CTL_IP:9292
	openstack endpoint create --region RegionOne \
	  image admin http://$CTL_IP:9292
}
...
glance_config_api () {
	...
	ops_add $glanceapifile database \
		connection mysql+pymysql://glance:$GLANCE_DBPASS@$CTL_IP/glance

	ops_add $glanceapifile keystone_authtoken auth_uri http://$CTL_IP:5000
	ops_add $glanceapifile keystone_authtoken auth_url http://$CTL_IP:5000
	ops_add $glanceapifile keystone_authtoken memcached_servers $CTL_IP:11211
...
glance_config_registry () {
	...
	connection mysql+pymysql://glance:$PASS_DATABASE_GLANCE@$CTL_IP/glance

	ops_add $glanceregistryfile keystone_authtoken auth_uri http://$CTL_IP:5000
	ops_add $glanceregistryfile keystone_authtoken auth_url http://$CTL_IP:5000
	ops_add $glanceregistryfile keystone_authtoken memcached_servers $CTL_IP:11211
