...
function neutron_config_server_component () {
	...
	ops_add $neutronfile DEFAULT transport_url rabbit://openstack:$RABBIT_PASS@$CTL_IP
	ops_add $neutronfile DEFAULT auth_strategy keystone
	#ops_add $neutronfile DEFAULT core_plugin ml2

	ops_add $neutronfile keystone_authtoken auth_uri http://$CTL_IP:5000
	ops_add $neutronfile keystone_authtoken auth_url http://$CTL_IP:5000
	ops_add $neutronfile keystone_authtoken memcached_servers $CTL_IP:11211
...
function nova_config () {
	...
	ops_add $novafile neutron url http://$CTL_IP:9696
	ops_add $novafile neutron auth_url http://$CTL_IP:5000
