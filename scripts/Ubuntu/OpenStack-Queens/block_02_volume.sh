...
function cinder_install_config() {
	...
	ops_conf_add $block_cinder_conf DEFAULT transport_url rabbit://openstack:$RABBIT_PASS@$CTL_MANAGE_IP
	ops_conf_add $block_cinder_conf DEFAULT auth_strategy keystone
	ops_conf_add $block_cinder_conf DEFAULT my_ip $CINDER_MANAGE_IP
	ops_conf_add $block_cinder_conf DEFAULT enabled_backends lvm
	ops_conf_add $block_cinder_conf DEFAULT glance_api_servers http://$CTL_MANAGE_IP:9292

        ops_conf_add $block_cinder_conf database connection mysql+pymysql://cinder:$CINDER_DBPASS@$CTL_MANAGE_IP/cinder

	ops_conf_add $block_cinder_conf keystone_authtoken auth_uri http://$CTL_MANAGE_IP:5000
	ops_conf_add $block_cinder_conf keystone_authtoken auth_url http://$CTL_MANAGE_IP:5000
        ops_conf_add $block_cinder_conf keystone_authtoken memcached_servers $CTL_MANAGE_IP:11211
