...
GRANT ALL PRIVILEGES ON nova_cell0.* TO 'nova'@'localhost' IDENTIFIED BY '$NOVA_CELL_DBPASS';
GRANT ALL PRIVILEGES ON nova_cell0.* TO 'nova'@'%' IDENTIFIED BY '$NOVA_CELL_DBPASS';
...
function nova_create_info () {
	...
	openstack endpoint create --region RegionOne compute public http://$CTL_MANAGE_IP:8774/v2.1
	openstack endpoint create --region RegionOne compute internal http://$CTL_MANAGE_IP:8774/v2.1
	openstack endpoint create --region RegionOne compute admin http://$CTL_MANAGE_IP:8774/v2.1
	...
	openstack endpoint create --region RegionOne placement public http://$CTL_MANAGE_IP:8778
	openstack endpoint create --region RegionOne placement internal http://$CTL_MANAGE_IP:8778
	openstack endpoint create --region RegionOne placement admin http://$CTL_MANAGE_IP:8778
}
...
nova_config () {
	...
	ops_add $novafile api_database \
		connection mysql+pymysql://nova:$NOVA_API_DBPASS@$CTL_MANAGE_IP/nova_api

	ops_add $novafile database \
		connection mysql+pymysql://nova:$NOVA_DBPASS@$CTL_MANAGE_IP/nova

	ops_add $novafile DEFAULT \
		transport_url rabbit://openstack:$RABBIT_PASS@$CTL_MANAGE_IP

	ops_add $novafile api auth_strategy keystone

	ops_add $novafile keystone_authtoken auth_uri http://$CTL_MANAGE_IP:5000
	ops_add $novafile keystone_authtoken auth_url http://$CTL_MANAGE_IP:5000
	ops_add $novafile keystone_authtoken memcached_servers $CTL_MANAGE_IP:11211
	...
	ops_add $novafile DEFAULT my_ip $CTL_MANAGE_IP
	...
	ops_add $novafile glance api_servers http://$CTL_MANAGE_IP:9292
	...
	ops_add $novafile placement auth_url http://$CTL_MANAGE_IP:5000/v3
