...
<html>
<head>
<META HTTP-EQUIV="Refresh" Content="0.5; URL=http://$CTL_MANAGE_IP/horizon">
</head>
<body>
...
horizon_config () {
	...
	sed -i 's/OPENSTACK_HOST = "127.0.0.1"/'"OPENSTACK_HOST = \"$CTL_MANAGE_IP\""'/g' $horizonfile

	echo "SESSION_ENGINE = 'django.contrib.sessions.backends.cache'" >> $horizonfile
	sed -i "s/'LOCATION': '127.0.0.1:11211',/""'LOCATION': '$CTL_MANAGE_IP:11211',""/g" $horizonfile
...
echo "URL: http://$CTL_MANAGE_IP/horizon"
echo "Domain: Default"
