...
function keystone_config () {
	...
	connection mysql+pymysql://keystone:$KEYSTONE_DBPASS@$CTL_MANAGE_IP/keystone
...
function keystone_bootstrap () {
	keystone-manage bootstrap --bootstrap-password $ADMIN_PASS \
	  --bootstrap-admin-url http://$CTL_MANAGE_IP:5000/v3/ \
	  --bootstrap-internal-url http://$CTL_MANAGE_IP:5000/v3/ \
	  --bootstrap-public-url http://$CTL_MANAGE_IP:5000/v3/ \
...
function keystone_config_apache () {
	...
	sleep 3
	echo "ServerName $CTL_HOSTNAME" >> /etc/apache2/apache2.conf
}
...
function keystone_create_domain_project_user_role () {
	...
	export OS_AUTH_URL=http://$CTL_MANAGE_IP:5000/v3
