...
function neutron_create_info () {
	...
	openstack endpoint create --region RegionOne network public http://$CTL_MANAGE_IP:9696
	openstack endpoint create --region RegionOne network internal http://$CTL_MANAGE_IP:9696
	openstack endpoint create --region RegionOne network admin http://$CTL_MANAGE_IP:9696
}
...
function neutron_config_server_component () {
	...
	ops_add $neutronfile database connection mysql+pymysql://neutron:$NEUTRON_DBPASS@$CTL_MANAGE_IP/neutron
	...
	ops_add $neutronfile nova auth_url http://$CTL_MANAGE_IP:5000
...
function neutron_config_metadata () {
	...
	ops_add $metadatafile DEFAULT nova_metadata_host $CTL_MANAGE_IP
