...
function swift_user_endpoint () {
	...
	openstack endpoint create --region RegionOne object-store public http://$CTL_MANAGE_IP:8080/v1/AUTH_%\(project_id\)s
	openstack endpoint create --region RegionOne object-store internal http://$CTL_MANAGE_IP:8080/v1/AUTH_%\(project_id\)s
	openstack endpoint create --region RegionOne object-store admin http://$CTL_MANAGE_IP:8080/v1/AUTH_%\(project_id\)s
}
...
function swift_config_proxy_server () {
	...
	ops_add $swiftfile "filter:authtoken" www_authenticate_uri http://CTL_MANAGE_IP:5000
	ops_add $swiftfile "filter:authtoken" auth_url http://CTL_MANAGE_IP:5000
	ops_add $swiftfile "filter:authtoken" memcached_servers CTL_MANAGE_IP:11211
...
function swift_ring () {
	...
	swift-ring-builder account.builder \
		add --region 1 --zone 1 --ip $OBJECT1_MANAGE_IP --port 6202 \
		--device $DEVICE_NAME1 --weight $DEVICE_WEIGHT1
        swift-ring-builder account.builder \
        	add --region 1 --zone 1 --ip $OBJECT1_MANAGE_IP --port 6202 \
        	--device $DEVICE_NAME2 --weight $DEVICE_WEIGHT2
        swift-ring-builder account.builder \
        	add --region 1 --zone 2 --ip $OBJECT2_MANAGE_IP --port 6202 \
        	--device $DEVICE_NAME1 --weight $DEVICE_WEIGHT1
        swift-ring-builder account.builder \
        	add --region 1 --zone 2 --ip $OBJECT2_MANAGE_IP --port 6202 \
        	--device $DEVICE_NAME2 --weight $DEVICE_WEIGHT2
	...
	swift-ring-builder container.builder \
  		add --region 1 --zone 1 --ip $OBJECT1_MANAGE_IP --port 6201 \
  		--device $DEVICE_NAME1 --weight $DEVICE_WEIGHT1
  	swift-ring-builder container.builder \
  		add --region 1 --zone 1 --ip $OBJECT1_MANAGE_IP --port 6201 \
  		--device $DEVICE_NAME2 --weight $DEVICE_WEIGHT2
  	swift-ring-builder container.builder \
  		add --region 1 --zone 2 --ip $OBJECT2_MANAGE_IP --port 6201 \
  		--device $DEVICE_NAME1 --weight $DEVICE_WEIGHT1
  	swift-ring-builder container.builder \
  		add --region 1 --zone 2 --ip $OBJECT2_MANAGE_IP --port 6201 \
  		--device $DEVICE_NAME2 --weight $DEVICE_WEIGHT2
	...
	swift-ring-builder object.builder \
  	...
  	sleep 3
  	scp /etc/swift/*.ring.gz root@$OBJECT1_MANAGE_IP:/etc/swift
        scp /etc/swift/*.ring.gz root@$OBJECT2_MANAGE_IP:/etc/swift
        cd /root/
}
...
function swift_config () {
	...
	curl -o /etc/swift/swift.conf \
  	...
  	cp $ctl_swift_conf $ctl_swift_conf.orig
        ...
        scp $ctl_swift_conf root@$OBJECT1_MANAGE_IP:/etc/swift
        scp $ctl_swift_conf root@$OBJECT2_MANAGE_IP:/etc/swift
        chown -R root:swift /etc/swift

        #ssh root@$OBJECT1_MANAGE_IP 'chown -R root:swift /etc/swift'
        #ssh root@$OBJECT2_MANAGE_IP 'chown -R root:swift /etc/swift'
}
