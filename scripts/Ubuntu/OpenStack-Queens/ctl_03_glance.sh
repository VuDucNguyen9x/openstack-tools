...
GRANT ALL PRIVILEGES ON glance.* TO 'glance'@'localhost' IDENTIFIED BY '$GLANCE_DBPASS' WITH GRANT OPTION;
GRANT ALL PRIVILEGES ON glance.* TO 'glance'@'%' IDENTIFIED BY '$GLANCE_DBPASS' WITH GRANT OPTION;
FLUSH PRIVILEGES;
EOF
}
...
glance_create_service () {
	...
	openstack endpoint create --region RegionOne \
	  image public http://$CTL_MANAGE_IP:9292
	openstack endpoint create --region RegionOne \
	  image internal http://$CTL_MANAGE_IP:9292
	openstack endpoint create --region RegionOne \
	  image admin http://$CTL_MANAGE_IP:9292
}
...
glance_config_api () {
	...
	ops_add $glanceapifile database \
		connection mysql+pymysql://glance:$GLANCE_DBPASS@$CTL_MANAGE_IP/glance

	ops_add $glanceapifile keystone_authtoken auth_uri http://$CTL_MANAGE_IP:5000
	ops_add $glanceapifile keystone_authtoken auth_url http://$CTL_MANAGE_IP:5000
	ops_add $glanceapifile keystone_authtoken memcached_servers $CTL_MANAGE_IP:11211
...
glance_config_registry () {
	...
	connection mysql+pymysql://glance:$PASS_DATABASE_GLANCE@$CTL_MANAGE_IP/glance

	ops_add $glanceregistryfile keystone_authtoken auth_uri http://$CTL_MANAGE_IP:5000
	ops_add $glanceregistryfile keystone_authtoken auth_url http://$CTL_MANAGE_IP:5000
	ops_add $glanceregistryfile keystone_authtoken memcached_servers $CTL_MANAGE_IP:11211
